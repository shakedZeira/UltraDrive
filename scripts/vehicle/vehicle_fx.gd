...
## collision sparks and a gated nitro-flame slot. Emitters are pre-built once
...
const SMOKE_AMOUNT_CAP := 120
const DUST_AMOUNT_CAP := 80
const SPARK_AMOUNT_CAP := 48
const NITRO_AMOUNT_CAP := 32
const TOTAL_BUDGET := SMOKE_AMOUNT_CAP + DUST_AMOUNT_CAP + SPARK_AMOUNT_CAP + NITRO_AMOUNT_CAP
...
func _build_emitters() -> void:
	...
	_emitters[CHANNEL_SMOKE] = _make_emitter(
		"FXSmoke", Color(0.70, 0.70, 0.75, 0.35), SMOKE_AMOUNT_CAP, false, Vector3(0.0, 0.4, 1.15)
	)
	_emitters[CHANNEL_DUST] = _make_emitter(
		"FXDust", Color(0.55, 0.45, 0.30, 0.60), DUST_AMOUNT_CAP, false, Vector3(0.0, 0.4, 1.15)
	)
...
func _make_emitter(node_name: String, color: Color, amount: int, one_shot: bool, local_pos: Vector3) -> GPUParticles3D:
